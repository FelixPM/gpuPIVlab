...
item=[0 item(2)+item(4) parentitem(3) 9];
...
item=[0 0 0 0];
parentitem=get(handles.multip02, 'Position');
item=[0 9+margin/2 parentitem(3) 23];
...
item=[parentitem(3)/4*0+margin item(2)+item(4)+margin/2 parentitem(3)/4 1.5];
handles.text6 = uicontrol(handles.uipanel6,'Style','text','units','characters','Horizontalalignment', 'left','position',[item(1) parentitem(4)-item(4)-margin-item(2) item(3)-margin item(4)],'String','X:');

item=[parentitem(3)/4*1-margin item(2) parentitem(3)/4 1.5];
handles.Increase_x = uicontrol(handles.uipanel6,'Style','edit','units','characters','position',[item(1) parentitem(4)-item(4)-margin-item(2) item(3)-margin item(4)],'String','','tag','Increase_x','Callback',@Increase_x_Callback);

item=[parentitem(3)/4*2+margin item(2) parentitem(3)/4 1.5];
handles.text7 = uicontrol(handles.uipanel6,'Style','text','units','characters','Horizontalalignment', 'left','position',[item(1) parentitem(4)-item(4)-margin-item(2) item(3)-margin item(4)],'String','Y:');

item=[parentitem(3)/4*3-margin item(2) parentitem(3)/4 1.5];
handles.Increase_y = uicontrol(handles.uipanel6,'Style','edit','units','characters','position',[item(1) parentitem(4)-item(4)-margin-item(2) item(3)-margin item(4)],'String','','tag','Increase_y','Callback',@Increase_y_Callback);

item=[0 item(2)+item(4)+margin/1.5 parentitem(3) 1.5];
handles.maskToSelectedGrow = uicontrol(handles.uipanel6,'Style','pushbutton','String','Grow current mask(s) to frames...','Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'Callback', @maskToSelectedGrow_Callback,'Tag','maskToSelectedGrow','TooltipString','Increase the mask that is currently displayed to other frames');
...
function maskToSelectedGrow_Callback(~, ~, ~)
handles=gethand;
filepath=retr('filepath');
if size(filepath,1) > 1
	str = strrep(get(handles.maskapplyselect,'string'),'-',':');
    ...
    if size(maskiererx{1,1},1) ~= 5
        selectionok=0;
		msgbox(['Use a square mask.'],'Error','error','modal')        
    end
    ...
    if selectionok==1
        ...
        increase_x=retr('increase_x');
        increase_y=retr('increase_y');

        for i=3:2:amount

            maskierery{1,i} = maskierery{1,i-1}  - [increase_y; increase_y; 0; 0; increase_y];
            maskierery{1,i+1} = maskierery{1,i-1}  - [increase_y; increase_y; 0; 0; increase_y];    
            maskiererx{1,i} = maskiererx{1,i-1}  - [increase_x; increase_x; 0; 0; increase_x];
            maskiererx{1,i+1} = maskiererx{1,i-1} - [increase_x; increase_x; 0; 0; increase_x];
	...
	for i=1:2:size(filepath,1)
		...
		if isempty(cancel)==1 || cancel ~=1
			...
			elseif get(handles.fftmulti,'Value')==1
				...
				[x, y, u, v, typevector] = piv_FFTmultiGPU2 (image1,image2,interrogationarea, step, subpixfinder, mask, roirect,passes,int2,int3,int4,imdeform,repeat,mask_auto);
				%u=real(u)
				%v=real(v)
			end
...
function Man_Increase_Callback
handles=gethand;
try
	x=round(str2num(get(handles.Increase_x,'String')));
	y=round(str2num(get(handles.Increase_y,'String')));

catch
end
if isempty(x)== 0 && isempty(y)== 0 && isnumeric(x) && isnumeric(y) 
	increase_x=x;
	increase_y=y;
	...
	put ('increase_x',increase_x);
    put ('increase_y',increase_y);

end

function Increase_x_Callback(~, ~, ~)
Man_Increase_Callback

function Increase_y_Callback(~, ~, ~)
Man_Increase_Callback
